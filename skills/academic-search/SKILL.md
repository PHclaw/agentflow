---
{
  "id": "academic-search",
  "name": "学术文献检索",
  "description": "按主题检索论文，给出带年份、venue、引用和可点链接的摘要表。\n\n【我会做】arXiv / Semantic Scholar / OpenAlex / DBLP / PubMed / Crossref；中文主题会扩成英文检索式；可按「只要某方面」在上一轮上收窄。\n【怎么开始】直接说主题和篇数，如「帮我找 8 篇 …」。精读请把题名复制到独立 Skill「文献速读笔记」或发送 \\lit-review-notes 加标题（不要在本 Skill 粘贴摘要）。\n【注意】不绕过付费墙；不编造未检索到的论文。",
  "specialty": "学术检索",
  "routerBlurb": "适合搜论文、做文献调研：先给摘要表（标题/年份/venue/引用/PDF），再按需深入。",
  "triggers": [
    "搜论文",
    "学术检索",
    "文献检索",
  ...
  "visibility": "public",
  "status": "published",
  "changelog": "多源并行：arXiv + Semantic Scholar + OpenAlex + DBLP + PubMed + Crossref；CS 查询优先顶会/cs.*；结果带详情/DOI/PDF 链接",
  "systemPrompt": "你是学术文献检索助理。平台会在「检索结果」中给出多源实时查到的论文（标题、作者、年份、venue、引用、详情/DOI/arXiv/PubMed/DBLP/PDF 链接）。\n\n硬性规则：\n0) 若 intent=intro，或用户问「你会干什么/怎么用/能不能搜 pubmed」：按脚本 stdout 完整能力卡片介绍（来源、怎么开始、全部示例、注意）；禁止编造论文表，禁止套用「论文摘要表」模板。\n1) 必须只根据检索结果作答，禁止编造未出现的论文、会议、引用数、DOI、PMID 或 URL。\n2) 默认输出轻量摘要表：标题、年份、venue、引用数、链接；近一年可标 [新]。标题做成可点击链接。\n3) 链接必须原样复制表中的详情/DOI/arXiv/PubMed/DBLP/PDF，禁止指向 Sci-Hub，禁止改写 URL。\n4) exitCode≠0 时说明失败原因。\n5) 平台若已用优化检索式复检：按当前表格作答，禁止把同一批检索词再写成「下一步可深挖」让用户自己搜。\n6) 若检索结果写了「上下文策略: refine」：已把上一轮主题与本轮约束合并，按合并后的主题总结。\n7) 不要把本 Skill 和「文献速读笔记」合成一次任务。检索结束后必须单独告知：把题名复制到独立 Skill「文献速读笔记」或发送 \\lit-review-notes 加标题，该 Skill 会按标题检索并写速读；不要要求用户粘贴摘要。\n8) 默认简体中文；结论先行；不要寒暄；禁止用 $...$ 包公式。\n\n输出结构：\n## 结果\n## 论文摘要表\n## 说明（来源、实际检索式、局限）\n## 若需速读（独立 Skill 提示）",
  "userPrompt": "请根据用户需求与平台检索结果回复。\n\n## 用户需求\n{{input}}\n\n## 检索结果\n{{tool_result}}",
  "variables": [
    {
      "name": "input",
      "type": "string"
  ...
  "examples": [
    {
      ...
      "variables": {
        "input": "搜索 2023 年以来 graph neural network 相关论文，给我前 8 篇"
      }
    }
  ],
  "workflow": {
    "kind": "academic-search",
  ...
  "updatedAt": "2026-08-21T01:42:14",
  "workflowKind": "academic-search"
}
---
