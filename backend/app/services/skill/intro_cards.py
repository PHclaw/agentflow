...
SKILL_CARDS: dict[str, SkillCard] = {
    ...
    "skill-1934395": {
        "name": "数据分析",
        "blurb": "从表格数据里做清洗、统计与异常识别，并写成带行动建议的分析报告。",
        "can": [
            "读取当前上传的表格（csv / xlsx 等）",
            "计算关键指标（均值、增长率、极值等）并找异常",
            "输出：数据概览、核心发现、趋势解读、行动建议",
        ],
        "how": "上传数据表并说明分析目的（例如「看本月销售同比」）。只分析本轮上传文件，不沿用历史里的旧表。",
        "examples": [
            "分析这份销售表，给出月度业绩报告",
            "找出异常订单并给出行动建议",
        ],
